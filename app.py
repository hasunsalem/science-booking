...
# --- 1. CONFIG & UI DESIGN (รวบรวม CSS ไว้ที่นี่ทั้งหมด) ---
...
<style>
    ...
    /* ชื่อระบบแบบเคลื่อนไหว */
    .animated-title {
        position: relative;
        z-index: 10;
        font-size: 3rem; /* ตรวจสอบแล้วว่าอยู่ใน String Block ไม่เกิด Syntax Error */
        font-weight: 900;
...
<div class="glass-header">
    ...
    <div class="inner-molecule" style="top:60%; left:80%; font-size:25px; animation-delay: -2s;">H₂SO₄</div>
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        u_name = st.text_input("ชื่อ - นามสกุล", key="user_name")
        u_stat = st.selectbox("สถานะ", ["นักศึกษา", "อาจารย์", "เจ้าหน้าที่"], key="user_stat")
        u_phone = st.text_input("เบอร์โทรศัพท์", key="user_phone")
    with c2:
        u_fac = st.text_input("คณะ/สังกัด", value="วท.บ.เคมี", key="user_fac")
        u_sup = st.text_input("อาจารย์ผู้ควบคุม", key="user_sup")
        u_purp = st.text_input("วัตถุประสงค์การใช้งาน", key="user_purp")
    ...
    with ct1: t_name = st.text_input("ชื่อเครื่องมือ", key="tool_name_in")
    with ct2: a_id = st.text_input("รหัสครุภัณฑ์", key="asset_id_in")
    ...
    with cd1: s_date = st.date_input("วันที่เริ่มใช้", min_value=datetime.today(), key="s_date")
    with cd2: e_date = st.date_input("วันที่คืน", min_value=s_date, key="e_date")
    st.markdown('</div></div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 ยืนยันคำขอจอง", use_container_width=True, type="primary"):
        if not u_name or not a_id:
            ...
            conflict = pd.read_sql_query("SELECT id FROM bookings WHERE tool_name = ? AND asset_id = ? AND approval_status != 'ไม่นุมัติ' AND (? <= end_date AND ? >= start_date)", 
                                         db_conn, params=(t_name, a_id, str(e_date), str(s_date)))
            if not conflict.empty:
                ...
                cur.execute("INSERT INTO bookings (name, status, phone, faculty, supervisor, purpose, start_date, end_date, tool_name, asset_id) VALUES (?,?,?,?,?,?,?,?,?,?)", 
                            (u_name, u_stat, u_phone, u_fac, u_sup, u_purp, str(s_date), str(e_date), t_name, a_id))
                db_conn.commit()
                st.success("🎉 บันทึกสำเร็จ!")
                st.balloons()

with tab2:
    st.markdown("### 📊 ตรวจสอบสถานะ (วท.บ.เคมี)")
    df = pd.read_sql_query("SELECT * FROM bookings ORDER BY id DESC", db_conn)
    if not df.empty:
        def style_status(row):
            s = row['approval_status']
            if s == 'อนุมัติแล้ว': return ['background-color: #d1fae5'] * len(row)
            elif s == 'ไม่นุมัติ': return ['background-color: #fee2e2'] * len(row)
