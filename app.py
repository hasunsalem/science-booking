...
<style>
    ...
    /* กล่องหน้าปก 3 มิติ สีเหลืองอ่อน พร้อมลายน้ำภายใน */
    .glass-header {
        position: relative; /* สำหรับให้ลายน้ำข้างในอ้างอิงตำแหน่ง */
        ...
        overflow: hidden; /* บล็อกลายน้ำไม่ให้ล้นออกนอกกล่อง */
    }
    
    /* ลายน้ำเฉพาะในกล่องหน้าปก */
    .inner-molecule {
        position: absolute;
        color: rgba(217, 119, 6, 0.15); /* สีส้มทองจางๆ */
    ...
    .animated-title {
        position: relative;
        z-index: 10; /* อยู่เหนือลายน้ำในกล่อง */
        font-size: 3rem;
        font-weight: 900;
...
# --- 3. APP INTERFACE ---
# หน้าปกสีเหลืองอ่อน 3 มิติ พร้อมลายน้ำเคลื่อนไหว "ภายในกล่อง"
st.markdown("""
<div class="glass-header">
    <!-- ลายน้ำเคลื่อนไหวในกล่อง -->
    ...
    <div class="inner-molecule" style="top:20%; left:70%; font-size:20px; animation-delay: -5s;">KMnO₄</div>
    <div class="inner-molecule" style="bottom:10%; left:15%; font-size:35px; animation-delay: -7s;">C₆H₁๒O₆</div>
...
with tab1:
    ...
    with col_t: tool_name = st.text_input("ชื่อเครื่องมือ")
    with col_i: asset_id = st.text_input("รหัสครุภัณฑ์")
    
    if tool_name and asset_id:
        df_busy = pd.read_sql_query("SELECT start_date, end_date FROM bookings WHERE tool_name = ? AND asset_id = ? AND approval_status != 'ไม่นุมัติ'", db_conn, params=(tool_name, asset_id))
    ...
    if st.button("🚀 ยืนยันการจอง", use_container_width=True, type="primary"):
        if not name or not asset_id:
            ...
            if not res.empty:
                st.error("❌ วันที่เลือกมีการจองอยู่แล้ว")
            else:
                cur = db_conn.cursor()
...
with tab2:
    ...
    is_admin = st.sidebar.checkbox("โหมด Admin")
